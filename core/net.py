...
class BaseNet(object):
    def __init__(self, config, random_seed, compute_grad):
        ...
        self.img_w          = config.CONST.IMG_W
        self.img_h          = config.CONST.IMG_H
        self.n_vox          = config.CONST.N_VOX
        self.n_gru_vox      = config.CONST.N_GRU_VOX
        self.compute_grad   = compute_grad

        self.x              = tensor5()         # (multi_views, self.batch_size, 3, self.img_h, self.img_w)
        self.y              = tensor5()         # (self.batch_size, self.n_vox, 2, self.n_vox, self.n_vox)
    ...
    def load(self, filename, ignore_param=True):
        ...
        for param_idx, param in enumerate(self.params):
            try:
                ...
                if ignore_param:
                    print('[WARN] Ignore index mismatched error')
                else:
                    raise
